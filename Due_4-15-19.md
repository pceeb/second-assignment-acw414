...
Then type ssh c177-*your login/class number*@hoffman2.idre.ucla.edu . 
Put in your password

2. What is the first thing that you do once you login?  What is the command?

You need to get from the login node into a compute node, which can be done by typing 'qrsh'
...
Ls -l lists directory contents using long list format . 
Ls -l -h produces a long list of the directory contents in a human-readable format (numbers are simplified, e.g. 4200 would become 4.2K)
...
Ls -R -t displays the contents of directories recursively arranged in order of when directories were most recently used (they are ordered in terms of when they were last used, starting with directories containing files/subdirectories that were most recently used). 
...
1. what is ~
This is a tilde and it represents your home directory. 
2. what is ../
This refers to the parent directory of the directory you are currently in. If you are inside a subdirectory, doing cd ../ will take you back a step in your path to the directory that contains your subdirectory. 
3. what is ./?
This refers to the directory you’re currently in. Doing cd ./ and then the path of subdirectories you wish to go down can take you from a parent directory to a nested subdirectory very quickly. 
...
````
$ cd . This will not do anything, it will keep Amanda into the directory she’s already in (data). 
$ cd / This moves Amanda to the root directory, making it her current working directory. 
$ cd /home/amanda  This will not work as the path for her home directory is /Users/amanda. If she doesnt have another directory in her current one called home with the subdirectory amanda inside it, this command will result in an error message. 
$ cd ../..  This will take her to /Users (as it will move her back 2 directories up the path).
$ cd ~ This will take her to her home directory
$ cd home   Unless Amanda has a directory within data called ‘home’, this command will result in an error message. This command would move her into a directory called ‘home’, but not her home directory
$ cd ~/data/.. This will take her back to her home directory. ~ takes her to her home directory, /data gets her back into the data directory, and /.. Takes her back to the parent directory of data, the home directory. 
$ cd  This will take amanda back to her home directory
$ cd ..  This will take her back to her home directory as .. takes her ‘up’ one level to the parent directory of the directory she’s currently in
````
---
### File systems
...
1. Using the filesystem diagram below, if pwd displays `/Users/thing`, what will `ls -F ../backup` display?
  ...
  d. `original/ pnas_final/ pnas_sub/`  <-- it will show this one
...
```
  ...
  b. `ls -r -F`  <--- this will show the output as they are already in the desired directory

  c. `ls -r -F /Users/backup` <--- this will also show the ouput, it just unnecessarily includes the path (which is not needed because they are already in the backup directory)
...
This created a file called my_file. It is a .txt file (a text file). It appears in my directory when using finder on mac.
...
You may want to create a file this way to write notes about your coding or for any time you want to save your work in hoffman2. You could also redirect the output of a command into a file such as this.

 Can you use touch to edit my_file.txt?

You cannot use touch to edit the file, but you can use nano
...
This symbol > overwrites what's in the file while >> appends whatever you want to echo to the end of the file. 
...
```
  ...
  c. The first three lines and the last two lines of animals.txt <---- this one
...
After creating and saving this file you realize you misspelled the filename! You want to correct the mistake, which of the following commands could you use to do so?
  ...
  b. `mv statstics.txt statistics.txt` <--- this one
...
This would delete your whole text file. It’s important to use -i as this shows a “remove thesis_backup/quotations.txt?” question before executing, and you have to type ‘y’ to delete the file. This is good as it could prevent you from deleting an important file (like the backup of your thesis quotations) accidentally!
...
```
Doing this copies the files into the given directory. In this example, amino-acids.txt and animals.txt are copied into the backup directory.  
...
```
This will produce an error message as you need to include a location to copy the files into. Typically, you can cp multiple files at once, you just need the last element to be a directory. Since this is not the case here, the command doesn't work. 
...
```
She needs to do mv *.dat analyzed/
...
$ mkdir 2016-05-20/data/processed  
$ mkdir 2016-05-20/data/raw  
^ this set of commands would achieve the objective, and does so by making subdirectories without leaving the current directory.  
...
$ cd data  
$ mkdir raw processed  
^ these commands would also achieve the objective as it makes all the required directories. The only differnce between this one and the first one is that you cd into the subdirectories to create the additional ones, while the previous set created them all without moving directories.  

$ mkdir 2016-05-20/data/raw  
$ mkdir 2016-05-20/data/processed  
^ this would not work as you need to create the parent directories (2016-05-20 and data) before creating the nested subdirectories.  

$ mkdir 2016-05-20  
$ cd 2016-05-20  
$ mkdir data  
$ mkdir raw processed  
^ this would not work as it creates the raw and processed directories inside 2016-05-20, rather than being inside data, nested inside 2016-05-20.  
...
This happens as -n arranges things in numerical order (smallest to largest) . 
...
This may have happened as data is often collected over a period of time so is sorted by date/time added. You could combine this with sort in order to sort entries alphabetically, so all duplicated lines are adjacent, then use uniq to remove them.   This could be done through: sort salmon.txt | uniq 
...
Cat animals.txt shows the whole animals.txt file.  
Head -n 5 shows the first 5 lines in animals.txt  
Tail -n 3 then takes the last 3 of these first 5 lines  
Sort -r > final.txt sorts these 3 lines in reverse alphabetical order, and saves the output in a .txt file called final.txt.  
final.txt should have the following inside it:  
2012-11-06,rabbit  
2012-11-06,deer   
2012-11-05,raccoon  
...
cut -d, -f 2 animals.txt | sort | uniq -c <--- this one
...
Yes, you can if you use the following commands:  
Ls *A.txt  
Ls *B.txt  
...
My expression would list the ones that end in A and B individually (after I execute each command). Meanwhile, the command from the exercise would have them all in one list.

Under what circumstances would your new expression produce an error message where the original one would not?  

My code would produce an error message if there were no files that ended in either A or B.
